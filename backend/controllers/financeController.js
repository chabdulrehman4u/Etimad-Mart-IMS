...
export const createPurchase = async (req, res) => {
  try {
    ...
    // Verify funding if paid cash or bank
    if (actualPaid > 0) {
      if (paymentMethod === 'cash') {
        const settings = await getOrCreateFinanceSettings();
        if (Number(settings.currentPettyCash || 0) < actualPaid) {
          return res.status(400).json({ message: 'Insufficient petty cash balance to pay for purchase' });
        }
      } else if (paymentMethod === 'bank') {
        if (!bankAccountId) {
        ...
        if (!bank || bank.currentBalance < actualPaid) {
          return res.status(400).json({ message: 'Insufficient bank balance to pay for purchase' });
        }
      }
    }

    // Create PurchaseBatch
    ...
    // Update Product stock & create stock history
    for (const item of normalizedItems) {
      const product = await Product.findById(item.productId);
      const previousStock = Number(product?.stock || 0);

      // Optionally update product originalPrice if provided
      await Product.findByIdAndUpdate(item.productId, {
        $inc: { stock: item.quantity },
        ...(item.unitPrice > 0 && { originalPrice: item.unitPrice }),
      });
    ...
    // If Paid via Cash/Bank: Cash/Bank -> Inventory (Net Worth is UNCHANGED)
    // If on Credit: Accounts Payable -> Inventory (Net Worth is UNCHANGED)
    if (actualPaid > 0) {
      await recordLedgerEntry({
        ...
        description: `Purchase from ${batch.supplierName} (${actualPaid.toLocaleString()} paid via ${paymentMethod})`,
        createdBy: req.user?.id,
    ...
    if (actualRemaining > 0) {
      await recordLedgerEntry({
        ...
        referenceType: 'purchase_batch',
        referenceId: String(batch._id),
        description: `Credit purchase liability for ${batch.supplierName} (Rs. ${actualRemaining.toLocaleString()})`,
        createdBy: req.user?.id,
        date: batch.purchaseDate,
      });
    }
