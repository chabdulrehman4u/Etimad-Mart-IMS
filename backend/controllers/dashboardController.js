...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    // Calculate whole stock worth (originalPrice * stock), retail worth, and total units in stock
    const stockWorthResult = await Product.aggregate([
      {
        $group: {
          _id: null,
          totalStockWorth: {
    ...
    const totalStockWorth = stockWorthResult.length > 0 ? stockWorthResult[0].totalStockWorth : 0;
    const totalRetailWorth = stockWorthResult.length > 0 ? stockWorthResult[0].totalRetailWorth : 0;
    const totalStockQuantity = stockWorthResult.length > 0 ? stockWorthResult[0].totalStockQuantity : 0;
